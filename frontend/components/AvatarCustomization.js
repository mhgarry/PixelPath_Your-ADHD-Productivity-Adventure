...
export default function PhaserGame() {
  ...
  function create() {
    ...
    // Define an animation if needed, or remove this if not needed
    this.anims.create({
      key: 'rotate',
      frames: this.anims.generateFrameNumbers('base', { start: 0, end: 63 }),
      frameRate: 24,
      repeat: -1,
    });

    this.base.play('rotate'); // Remove this if 'rotate' is not needed
  }
  ...
  const saveCharacter = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/avatar/save`, {
        method: 'POST',
        headers: {
